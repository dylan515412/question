...
function normalizeDashboardState() {
  ...
  state.daily.forEach((entry) => {
    ...
    entry.points = getEntryPoints(entry);
  });
  ...
  state.score = Math.max(
    0,
    [...state.daily, ...state.words].reduce((total, entry) => total + getEntryPoints(entry), 0) -
      Number(state.redeemedSurprises || 0) * 100
  );
}
...
function mergeCloudState(remoteState) {
  ...
  state.coverPhotos = mergeByIdentity(
    state.coverPhotos,
    remoteState.coverPhotos,
    "cover",
    (photo) => photo.src || photo.photo || photo.photoId || photo.id
  );
...
async function renderCoverPhotos() {
  if (!state.coverPhotos.length) return;
...
function getEntryPoints(entry) {
  return Number.isFinite(entry.points) ? entry.points : 2;
}

function hasDailyScoreForDate(dateKey) {
  return state.daily.some((entry) => inferDateKey(entry) === dateKey && getEntryPoints(entry) > 0);
}

function getEntryPointsForType(type, dateKey) {
  if (type !== "daily") return 2;
  return hasDailyScoreForDate(dateKey) ? 0 : 2;
}
...
function updateScore() {
  ...
  dailySubmitButton.textContent = hasDailyScoreForDate(toDateKey(new Date())) ? "记录" : "+2";
